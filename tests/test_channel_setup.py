...
def test_secret_save_never_round_trips_and_requires_restart(app_client, install_token, tmp_path):
    ...
    # Not a mode-bit assertion: os.chmod cannot express one on Windows, where it
    # reports success and leaves the inherited ACL in place. Ask whether the file
    # is in fact owner-only, on whichever platform this is running.
    assert config.owner_only(tmp_path / "cfg" / "channel_secrets.json")
